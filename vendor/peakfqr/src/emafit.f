...
c                                          returned in /tac005/...,as_G_PRL             
c                                        renamed to pseudo effective record
c                                        length 15 May 2024 by SAS
c
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
...
c            dtype(n)   i*4  vector describing input data
c                              dtype(i) = 0 => systematic data
c                              dtype(i) = 1 => historic data
...
!    1               reg_SD,reg_SD_mse,r_G,r_G_mse,gbthrsh0,
!    1                  cmoms,pq,nq,yp,ci_low,ci_high,var_est)
!
!     implicit none
!
!     integer
!    1  i,n,nq,nqd, dtype(*)                                   ! input variables
!     
!     double precision
...
c            eps        r*8  ci coverage (usually just 0.90)
c            wght_opt_n i*4  Integer corresponding to weighting option for regional skew.
c                            Choices are:
...
c       output variables:
c       ------------------------------------------------------------------------
c            gbval      r*8  MGBT low outlier value (added by SAS)
c            gbns       i*4  Number of "systematic" peaks. Peak is considered 
c                            "systematic" if it is used in the MGBT
c            gbnzero    i*4  Number of zero flows 
c            gbnlow     i*4  Number of low outliers detected
c            cmoms(3,3) r*8  first 3 central moments
c                             (,1) using regional info and at-site data
c                             (,2) using just at-site data
c                             (,3) using B17B formula for at-site MSE(G)
...
c
     ...
     2               gbthrsh0,pq,nq,eps,wght_opt_n,
     3               gbval,gbns,gbnzero,gbnlow,gbp,
     4               gbqs,as_G_mse_o,as_G_mse_Syst_o,
     5               as_G_PRL_o,cmoms,yp,ci_low,ci_high,var_est, Wdout,
     6               qlema,quema,tlema,tuema,nu)

      implicit none

      integer
     1  i,n,nq,nqd,ns,nlow,nzero,gbns,gbnlow,gbnzero,dtype(*),dtype2,
     2  wght_opt_n  
     ...
     2  cmoms(3,3),pq(*),yp(*),ci_low(*),ci_high(*),var_est(*),nu(*),! output vars
     3  pqd,eps,gbthrsh0,gbcrit,gbthresh, pvaluew, qs, 
     4  gbval, gbp(20000), gbqs(20000), Wd, Wdout,
     5  as_M_mse,as_S2_mse,as_G_mse,as_G_mse_Syst,as_G_PRL,
     6  reg_M,reg_M_mse,r_M,r_M_mse,r_S2,r_S2_mse,
     7  as_M_mse_o,as_S2_mse_o,as_G_mse_o,as_G_mse_Syst_o,as_G_PRL_o, 
     8  ql2,qu2,tl2,tu2,qlema(nx),quema(nx),tlema(nx),tuema(nx)
...
!      common /tacci1/eps
     ...
     1               ns,nlow,nzero
      common /tacg02/ql2(nx),qu2(nx),tl2(nx),tu2(nx),dtype2(nx) !MGBT adjusted data
      common /tac005/as_M_mse,as_S2_mse,as_G_mse,as_G_mse_Syst,as_G_PRL
      common /sas/Wd !shame, shame
...
!     Set option for regional skew weighting from numeric input option 
      if( wght_opt_n .eq. 1) then
      !Halloween method weighting from Greg Schwarz
      weightOpt =  "HWN"

      else if(wght_opt_n .eq. 2) then

      weightOpt = "ERL" !Relative effective record length weighting
      
      else
      weightOpt = "" !Bulletin 17C weighting algorithm
      endif


        call emafitb(n,ql,qu,tl,tu,dtype,
     1                  r_M,r_M_mse,r_S2,r_S2_mse,r_G,r_G_mse,
     1                  eps,gbthrsh0,pq,nq,weightOpt,
     1                  cmoms,yp,ci_low,ci_high,var_est,nu)
c
c     correction to adjust for small sample sizes (see tac notes 17 feb 2007)
c
      do 20 i=(nq-1)/2,1,-1
...
30    continue
      ...
      gbval =  gbcrit !SAS: get that pesky MGBT value out of common block
      gbp = pvaluew
      gbqs = qs
      gbns = ns !number of "systematic" flows
      gbnlow = nlow
      gbnzero = nzero
      as_M_mse_o = as_M_mse
      as_S2_mse_o = as_S2_mse
      as_G_mse_o = as_G_mse
      as_G_mse_Syst_o = as_G_mse_Syst
      as_G_PRL_o = as_G_PRL
      Wdout = Wd
      qlema = ql2
      quema = qu2
...
c
     ...
     1                   r_M,r_M_mse,r_S2,r_S2_mse,r_G,r_G_mse,
     1                   eps,gbthrsh0,pq,nq,weightOpt,
     2                   cmoms,yp,ci_low,ci_high,var_est,nu)
     ...
     1  ntmax,nn,nx
      ...
      integer
     1  n,i,nt,ns,nlow,nzero,nGBiter,nlow_V,it_max,nq,i1,dtype_in(*)
     
      double precision
     1  ql_in(*),qu_in(*),tl_in(*),tu_in(*),
     1  r_M,r_M_mse,r_S2,r_S2_mse,r_G,r_G_mse,pq(*),                 ! input
     2  cmoms(3,3),yp(nn),ci_low(nn),ci_high(nn),nu(nn),var_est(nn),     ! output
     3  yp1(nn),yp2(nn),ci_low1(nn),ci_low2(nn),ci_high1(nn),
     4  ci_high2(nn),nu1(nn),nu2(nn),skewmin,qP3,parms(3),
     5  ql,qu,tl,tu,qs,thrmin(1),thrmax(1),
     6  cv_yp_syp(2,2,nn),eps,tl2(ntmax),tu2(ntmax),nobs(ntmax),
     7  skew,wt,as_M_mse,as_S2_mse,as_G_mse,as_G_mse_Syst,as_G_PRL,
     8  eff_n(1),gbthrsh0,gbcrit,gbthresh,gbcrit_V,gbthresh_V,pvaluew
     ...
     1  mseg_all,mse_ema, detrat, Wd, erlg
      ...
      character*4 
     1  at_site_option,at_site_default,at_site_std,
     2  dtype,VarS2opt, weightOpt
...
!   loop up to 10 times to see if any new LOs uncovered
...
!   SAS 2023-09-13: Code below should be equivalent to PeakFQ 7.4.1
!   (as long as nothing weird like a single GB test is attempted). 


!      if(gbtype .eq. "FIXE" .and. nlow .gt. 0) then
      if(gbthrsh0 .le. -6 .and. nlow .gt. 0) then
        at_site_std = "B17B"
      else
...
c
        eff_n(1) = dble(max(ns,10))    ! use a reasonable sample size if ns=0
        as_G_mse_Syst = mseg_all(i1,eff_n,thrmin,thrmax,cmoms(1,2)) ! ERL Computation
        as_G_PRL    = dble(eff_n(1))*(as_G_mse_Syst/as_G_mse)
...
!     weightOpt = "HWN"
      ...
      !Wd = erlg(r_G, as_G_mse) / n !Relative effective record length weighting
...
c
     ...
     3         yp,cv_yp_syp,ci_low,ci_high, nu)
...
c
     ...
     3         yp1,cv_yp_syp,ci_low1,ci_high1, nu1)
     ...
     1         r_s2,
     2         r_m_mse, r_s2_mse, r_g_mse,
     3         yp2,cv_yp_syp,ci_low2,ci_high2, nu2)
...
c            gbthrsh0   r*8  critical value for Grubbs-Beck test
c                              (<= -99 -> estimate threshold)
c                              (>-99 but < -6 -> no low outlier test)
...
C JFE      common /tacg01/gbcrit,gbthresh,pvaluew(10000),qs(10000),
     ...
     1               ns,nlow,nzero,gbtype
      ...
      data gbtmin/-6.d0/,lmissing/-20.d0/
...
c  note that zero flows are automatically treated as low outliers in MGBT
      ...
      do 10 i=1,n
        if(ql_in(i) .eq. qu_in(i) .and. dtype(i) .eq. 0) then ! N.B. Criterion
          ns = ns + 1
          x(ns) = ql_in(i)
          qmin  = min(qmin,x(ns))
          qs(ns)= 10**x(ns)
...
c
      ...
      gbtype = "MGBT" ! Hard code MGBT (SAS 2023-01-30)

c no low outlier test
      if(gbthrsh0 .lt. -6 .and. gbthrsh0 .gt. -99) then
        gbthresh  = -99.d0        
        gbcrit    =  gbthresh        
        goto 25
      endif
...
c specified low outlier threshold?
      ...
      if( (ns-nzero) .le. 5) then    ! not enough data>0 to apply low-outlier test
        error stop 'inadequate data for grubbs-beck test'
...
c                            effective record length with Halloween method
c
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
c       output variable: 
...
c
     ...
     1     r_M,     r_S2,     r_G,
     3     r_M_mse, r_S2_mse, r_G_mse, Wd,Wa,
     4     moms_out(3),
...
c           SAS 12/7/2021: Implement B17C EQ 7-10
c           moms(3,i) = moms(3,i)
...
!             write(*,*) i,(moms(j,i),j=1,3),d11(i)
40          continue
             error stop 'Error in p3est_ema: failure to converge'
      end
...
c       2004 gets Griffis et al. [2004] -- the default!
      ...
      if(n .gt. nsize) then
        error stop '(moms_p3): sample size too large '
      endif
...
c     SAS 12/20/2021 nthresh used in first call below to compute actual
c     EMA MSE. n_adj is used in the second call because it corresponds
c     to the n_adj used for the B17B MSE from mseg() used later in the 
c     calculation. 

        bias_adj = mse_ema(nthresh,nobs,tl,tu,mc,3) /
     1             mse_ema(i1,dble(n_adj),INF(1),INF(2),mc,3)
        
!     SAS 12/20/2021 Limit bias_adj to prevent censoring from decreasing MSE
!     SAS 9/13/2023 Turned this back off based on Greg's analysis
!          if(bias_adj .lt. 1.d0) then
!               WRITE(*,*) "Skew adjustment less than unity:", bias_adj
!               WRITE(*,*) "Skew MSE has been forced to B17B value"
!          endif

!        bias_adj = max(bias_adj, 1.d0)
        
        mseg_all = bias_adj * mseg(n_adj(1),mc(3))
      else  
          ERROR STOP 'Invalid input to mseg_all'
      endif
...
c   test input value of kmom     
      if(kmom .gt. 3 .or. kmom .lt. 1) then
        error stop 'Error in mse_ema, kmom must be between 1 and 3'
      endif
...
c Consider moving mse_i eqn to separate function 
        ...
        if(i .eq. 100) then
          error stop ' failure to converge in erlg'
        endif
...
c
     ...
     1    (yp,cv_yp_syp,eps,ci_low,ci_high,nu)
...
C  CHECK ARRAY BOUNDS
      IF(NND(1)*NND(2)*NND(3) .GT. PDIM) THEN
        ERROR STOP ' NEED TO INCREASE ARRAY SIZES (PDIM) IN MC2MNVB'
      ENDIF
...
C  CHOLESKY DECOMPOSITION (USED TO INTRODUCE PROPER COVARIANCE)
	  ...
	  IF(IFLAG .NE. 0)  THEN
	    ERROR STOP 'MATRIX S NOT POS-SEMI-D; (CHOL33)'
	  ENDIF
...
c
        ...
        if(r_G_mse .gt. 0.d0) then   ! "WEIGHTED"
		   !Why does this line call mseg_all() instead of just using the value in s_mc(3,3)
		   !This makes no sense
		   !Worse news is, confidence intervals change noticeably if behavior is changed
...
!           Check added by SAS 2024-05-09
!           Needed for situations where MGBT changes tl 
!           to greater than input tu for one or more years
            if(tl > tu) then
                error stop 'Invalid perception threshold tl > tu'
            endif
c
...
C
     ...
     2  XMIN,XMAX,MCT(3),S_MCT(3,3),S
     
      INTEGER
     1  I,J,IFAIL,NND(3),NND_OUT(3),STERM(6)
      ...
      DATA EPS/0.D0/,ACC/0.D0/,DELTA/0.0001D0/,STERM/2,3,1,4,2,0/
        ...
        CALL DMCOPY(3,3,S_MC,3,3,3,S_MCT,3)
          S = SQRT(MC(2))
      DO I=1,6    !  I INDEXES PC VAR WRT DERIV OF PN COMPUTED
        DO J=1,6  !  J INDEXES PN VAR WHOSE CHANGE IS TO BE COMPUTED
            XMAX = DELTA * S**STERM(I) ! Covariance ~ S^I
            XMIN = -XMAX
      CALL DIFF2(MCT,S_MCT,I,J,NND_OUT,1,0.D0,XMIN,XMAX,EPS,
     1            ACC,JAC(J,I),ERROR,IFAIL) !q&d
        END DO
      END DO
...
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
       SUBROUTINE JACF(MC,S_MC,X0,NND,I,J,X,FX)
!===============================================================================
!
!        COMPUTES JACOBIAN OF SIGMA(M1,M2,M3) WRT SIGMA(M,S2,G)
!        FOR ALGEBRA, SEE COHN [2012] (NOTEBOOK 3)
!          
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
!
!        INPUT VARIABLES:
!        -----------------------------------------------------------------------
!            MC(3)      R*8  PARAMETERS OF LP3 DISTRIBUTION {M1,M2,M3}
!            S_MC(3,3)  R*8  COVARIANCE OF NON-CENTRAL MOMENTS
!            X0         R*8  INITIAL DELTA VALUE
!            NND(3)     I*4  NUMBER OF NODES IN QUADRATURE ALONG EACH DIMENSION
!            I          I*4  ELEMENT OF TRIDIAGONAL FOR DIFFERENTIATION
!            J          I*4  ELEMENT OF TRIDIAGONAL TO RETURN
...
!       OUTPUT
!       ------------------------------------------------------------------------
!            X          R*8  FINAL DELTA VALUE
!           FX          R*8  RETURNED TRIDIAGONAL VALUE
!
!
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
!
...
!
      IMPLICIT NONE
      INTEGER I,J,NND(3),N,IFLAG
      
      DOUBLE PRECISION MC(3),S_MC(3,3),X0,S_MCT(3,3),S_MN(3,3)
      DOUBLE PRECISION V(3,3),PC(6),PN(6),X,FX
      ...
      DO N=1,20
        ...
        CALL CHOL33(S_MCT,V,IFLAG)

        IF(IFLAG .NE. 0) THEN 
          X = X/10.d0 !Try smaller X
        ELSE 
          EXIT !Cholesky works and can continue
        END IF
         
      END DO

      IF(IFLAG .NE. 0) THEN
        ERROR STOP 'Error in JACF'
      ENDIF
...
5       continue
      ...
      if(n .gt. 12) then
        error stop 'mP3: error; n too large (greater than 12)'
      endif
c
...
c
      ...
      dp3g = 0 !Initialized by SAS
      dp3wh = 0 !Initialized by SAS
...
c
      ...
      pP3g = 0 !Initialized by SAS
      pP3wh = 0 !Initialized by SAS
...
30    continue
      ...
      subroutine s_kfxx(skew,prob,k)
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
c    wrapper for kfxx function for calling from R
c
c    seth siefken........14 Sep 2023
c
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
c
c       input variables:
...
c            prob       r*8  flood non-exceedance probability 
c
c****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
...
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
      ...
      IF (abs(MC(3)) .lt. 0.04 ) THEN
!     If skew is very close to zero, then determinate ratio simplifies to unity
      detrat = 1d0
      ...
      !Compute EQ 16 in Greg's derivation
...
!
!    Computes skew weighting factor using determinate ratio for Greg Schwarz's
!    Halloween weighting method. This subroutine wrapper allows access 
!    from R. 
!
!    Seth Siefken 2023-12-22
!
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
!
!       input variables:
...
!       output variables:
!       ---------------------------------------------------------------------------
!            detratout   r*8 determinate ratio for skew weighting
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
      ...
      double precision detrat, mc(3), tl(*), tu(*), nobs(*), detratout

      detratout = detrat(mc, n, nthresh, nobs, tl, tu)
...
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
!
!    Computes the inverse CDF of a Pearson Type III variate with moments 
!    m=(mu,sigma^2,g) This subroutine wrapper allows access from R. 
!
!    Seth Siefken 2024-03-26
!
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
!
!       input variables:
!       ---------------------------------------------------------------------------
!             q      r*8     probability 
!             m(3)   r*8     central moments vector
!
!       output variables:
!       ---------------------------------------------------------------------------
!           qP3out   r*8     quantile estimate
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
      
      implicit none
      
     
      double precision qP3, q, m(3), qP3out
...
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
!
!    Computes the mse of station skew for the bulletin 17b/17c analysis
!    with the mseg_all() function. This subroutine wrapper allows access from R. 
!
!    Seth Siefken 2024-03-28
!
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
!
!       input variables:
!       ---------------------------------------------------------------------------
!          nthresh    i*4  number of distinct censoring thresholds ( (a,b) pairs)
!          nobs(*)      r*8  vector of number of observations corresponding 
!                            to threshold pair (tl(i),tu(i))
!            tl(*)        r*8  vector of lower bounds (a)
!            tu(*)        r*8  vector of upper bounds (b)
!            mc(3)        r*8  vector of estimated lp3 moments
!       output variables:
!       ---------------------------------------------------------------------------
!           mseg_all     r*8  mse of at-site (station) skew
!****|===|====-====|====-====|====-====|====-====|====-====|====-====|==////////
      
      implicit none
      
      integer nthresh
      double precision nobs(*),tl(*),tu(*),mc(3), mseg, mseg_all
     

      mseg = mseg_all(nthresh,nobs,tl,tu,mc)
